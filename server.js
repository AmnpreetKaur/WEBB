...
cloudinary.config({
    cloud_name: 'dwwjlqpbg',
    api_key: '833936367755169',
    api_secret: 'skcap5kETRGIN0uuJ7SQOu7vUgk',
    secure: true
});
...
app.get("/posts", ensureLogin, function(req,res){
    if(req.query.category) {
        blog.getPostsByCategory(req.query.category).then((data) => {
            console.log("getpostsbycategory function");
            if(data.length > 0){
        ...
        blog.getPostsByMinDate(req.query.minDate).then((data) => {
            console.log("getpostsbydate function");
            if(data.length > 0){
                res.render("posts",{posts: data});
            }
            else{
...
app.get("/post/:value", ensureLogin, function(req,res) {
    blog.getPostById(req.params.value).then((data) => {
        console.log("getpostsbyid function");
        res.json({data});
    }).catch((err) => {
...
.then(() => {
    ...
    console.log("Initialize Rejected");
})
